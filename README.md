...
The Power BI report uses the CSV extracts in `powerbi/` and does not train the model. It is organised as a star schema with a prediction fact table and date and horizon dimensions.

| File | Rows | Purpose |
| --- | ---: | --- |
| `fact_predictions.csv` | 26,208 | One row per forecast time and horizon |
| `dim_date.csv` | 91 | Dates and Irish bank holidays |
| `dim_horizon.csv` | 6 | Horizon-level scores |
| `scores_long.csv` | 30 | Model, horizon, and metric scores |

Regenerate the extracts with:
...
The test suite validates the feature-engineering and time-splitting guarantees.

### View the existing dashboard

The committed `dashboard.html`, `results/`, and `powerbi/` outputs come from a completed run, so no download is required to inspect them. Serve the project folder locally:
...
`fetch_history.py` is slow but resumable. Re-running it may download newer months, so results can change as the available history grows.

## Keeping it current

`.github/workflows/update.yml` runs every morning at 06:20 UTC. It fetches new data, runs the tests, retrains the models, rebuilds the dashboard and Power BI extracts, and commits the updated outputs. Cached historical months mean a daily run normally downloads only new data.

The web dashboard is designed to be served from the repository root by GitHub Pages.
